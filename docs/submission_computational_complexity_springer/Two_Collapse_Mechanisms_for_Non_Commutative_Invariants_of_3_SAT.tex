...
% Manuscript: Impossibility of Local Quaternionic Invariants for 3-SAT Separation
% Springer Computational Complexity submission (sn-jnl working format)
...
% Running head (Springer submission convention)
\makeatletter
...
\newcommand{\QILRepositoryURL}{https://github.com/mauramyrez/QIL-Research-Framework}
...
\title[Quaternionic Obstruction Collapse]{Impossibility of Local Quaternionic Invariants for $3$-SAT Separation: Intrinsic Gauge Collapse and Terminal Abelianization}
\titlerunning{Quaternionic Obstruction Collapse for 3-SAT}
...
\abstract{We prove exact \emph{boundary theorems} for a natural class of local,
low-degree non-commutative invariants built over the rational Hamiltonian
quaternions $\HH(\QQ)$---the non-commutative lift of spectral and
Dieudonn\'e-type readouts used in matrix-semi-invariant obstruction
searches. On exactly labelled $3$-CNF instances (our combinatorial testbed,
not a $\mathrm{P}$ versus $\mathrm{NP}$ claim), two successive encodings
exhibit mechanistically distinct collapse: (i)~bipartite bilinearity forces an
intrinsic diagonal gauge, commuting the Dieudonn\'e representative; (ii)~after
the minimal arity lift to an order-$3$ tensor, any scalar certificate
factoring through the reduced norm projects word-traces onto the conjugation
invariant pair $(\Trd,\Nrd)$, erasing oriented non-commutative data at the
readout. We give a taxonomy---object collapse versus measurement
collapse---and necessary conditions for any successor invariant. All
constants are machine-checked in the open-source \emph{Quaternionic Invariant
Laboratory} (QIL) over exact $\QQ$ arithmetic.}

\keywords{algebraic complexity, non-commutative invariants, matrix semi-invariants, Dieudonn\'e determinant, impossibility results, division rings, exact arithmetic}
...
\subsection{Non-commutative invariants in algebraic complexity}

A recurring strategy in algebraic complexity and representation-theoretic
obstruction search is to attach to a computational object a polynomial or
matrix datum over a structured coefficient ring, then seek a \emph{low-degree,
basis-independent} functional that separates two classes of instances---for
example, orbit closures in geometric complexity theory
(GCT)~\cite{GCT1,GCT2,MulmuleyCACM}, matrix semi-invariants of quiver
representations~\cite{DerksenMakam}, or rational certificates from
non-commutative data~\cite{GGOV,IvanyosQiaoSubrahmanyam}. When such a
functional exists, it is a candidate \emph{obstruction}; when a natural
family provably cannot separate, the failure mode itself delimits what any
successor must avoid.

This paper is a \emph{boundary result} in that spirit. We study local,
low-degree invariants over the rational Hamiltonian quaternion division ring
$\HH(\QQ)$---the natural non-commutative lift of spectral and Dieudonn\'e-type
readouts on incidence data---and prove exact structural theorems explaining
why a concrete pipeline cannot yield a separating $\QQ$-valued certificate.
We do \emph{not} claim a separation of $\mathrm{P}$ from $\mathrm{NP}$, a
new theorem about algebrization~\cite{AW}, or progress toward explicit GCT
obstructions; our contribution is a rigorously proved impossibility theorem
for a specific, \emph{a priori} reasonable family of non-commutative
candidates, together with a taxonomy of the two distinct mechanisms by which
such candidates degenerate.

\paragraph{Subject classification.}
68Q15 (Complexity lower bounds); 68Q17 (Algebraic complexity theory);
16W10 (Associative rings and algebras over fields); 15A15 (Matrix theory).

\subsection{Combinatorial testbed: exactly labelled \texorpdfstring{$3$-SAT}{3-SAT}}

We instantiate the general framework on $3$-CNF
formulas~\cite{Cook,GareyJohnson}: finite combinatorial objects with an exact
satisfiability label. This testbed lets every invariant be evaluated over
$\ZZ$ and $\QQ$ with no numerical error and lets negative results be proved,
not merely observed on heuristics. Any functional that decided satisfiability
by thresholding a low-complexity invariant would be extraordinary---and, by
the natural-proofs framework~\cite{RR}, unexpected in the form we study. Our
aim is to show \emph{structurally and exactly} why the quaternionic pipeline
we build cannot serve as such a separator.

\subsection{Two collapse mechanisms: a preview}

We report two distinct degeneration phenomena at two levels of the
construction---complementary poles of a single taxonomy.

\begin{enumerate}
\item \textbf{Intrinsic gauge collapse} (matrix level,
Sections~\ref{sec:incidence}--\ref{sec:collapse}). A bipartite
clause--variable incidence matrix is a bilinear object on a two-colourable
graph. The two-colouring \emph{is} a diagonal gauge involution
$\sigma_{\bk}(\Aphi)=G\Aphi G^{-1}$; it conjugates the Dieudonn\'e
representative into a commutative subfield and annihilates literal commutator
torsion identically. Non-commutativity is destroyed \emph{in the object}.

\item \textbf{Extrinsic terminal abelianization} (tensor level,
Sections~\ref{sec:tensor}--\ref{sec:terminal}). A symmetric order-$3$ tensor
removes the bipartite gauge and can retain genuine non-commutative structure.
Yet any scalar readout factoring through the reduced norm---Frobenius
reduced-norm sums or cyclic word-trace certificates---projects through the
abelianization $\HH(\QQ)^{\times}\to\QQ_{>0}$ onto the centre. Non-commutativity
survives in the object but is erased \emph{by the measurement}.
\end{enumerate}

Section~\ref{sec:bridge} proves the first collapse intrinsic to all
two-colourable order-$2$ formats and that escaping it forces arity three,
making the second phase inevitable.

Linear invariants over commutative fields are often \emph{gauge trivial}:
spectral data are invariant under $A\mapsto U^{\dagger}AU$. This illustrates,
in miniature, a phenomenon we compare---only by structural analogy, without
claiming a formal reduction---to how algebrization defeats certain
algebraic-degree arguments~\cite{AW}; we do not prove any statement about
the algebrization barrier itself. Encoding literal polarities as
anticommuting units in $\HH(\QQ)$ was an \emph{a priori} reasonable probe
because it sits in the same Dieudonn\'e and semi-invariant toolbox cited
above; that this specific pipeline fails with an exact explanation is what
makes the outcome informative for obstruction search.

\subsection{Main results and scope}\label{sec:contributions}

\begin{enumerate}[label=(\roman*)]
\item \textbf{Intrinsic bipartite gauge collapse.}
Theorem~\ref{thm:collapse} and Theorem~\ref{thm:intrinsic}: every
non-singular order-$2$ encoding on a two-colourable support with literal
polarities in the $(-1)$-eigenspace of $\sigma_{\bk}$ has
$\Ddet(A)\in\QQ[k]$; commutator torsion vanishes identically.
\item \textbf{Minimal arity lift.}
Corollary~\ref{cor:necessity}: faithful width-$3$ clause encoding without
auxiliary vertices requires a symmetric order-$3$ tensor, removing the
bipartite gauge (Proposition~\ref{prop:nogauge}).
\item \textbf{Terminal abelianization.}
Theorem~\ref{thm:terminal}: for any marginal from linear contraction and
any $F=g\circ\Nrd$, the certificate $F(\Tr(M^p))$ depends only on
$(\Trd,\Nrd)$; oriented non-commutative data are erased at the readout
(Remark~\ref{rem:terminal} locates this step in the pipeline).
\item \textbf{Taxonomy and escape conditions.}
Section~\ref{sec:discussion} formulates necessary conditions for successor
invariants (non-abelian boundary evaluation or non-local contraction).
\item \textbf{Exact verification.}
Section~\ref{sec:experiments} corroborates the structural predictions on
size-matched instances; Section~\ref{sec:software} documents QIL, the
open-source exact-arithmetic laboratory in which every constant in this
paper is machine-checked.
\end{enumerate}

\subsection{Organization}

Sections~\ref{sec:incidence}--\ref{sec:collapse} develop the quaternionic
incidence matrix and intrinsic collapse; Section~\ref{sec:bridge} proves the
arity obstruction; Sections~\ref{sec:tensor}--\ref{sec:contract} build
the order-$3$ tensor, Fourier annihilation, and contraction; Section~\ref{sec:terminal}
states the terminal abelianization theorem; Section~\ref{sec:experiments}
reports exact size-matched data; Section~\ref{sec:discussion} draws
implications for obstruction search; Section~\ref{sec:software} describes QIL.

\medskip
\noindent\textbf{Exact arithmetic.}
All computations use arbitrary-precision $\ZZ$ and $\QQ$ via QIL
(Section~\ref{sec:software}); there is no floating point anywhere.
...
quaternion algebra over the rationals~\cite{Voight,Lam}, with multiplication
...
We elevate the readout failure to a structural \emph{delimitation theorem}:
any scalar extraction factoring through the reduced norm collapses the
non-commutative spectrum of the contracted matrix onto a conjugation-invariant
central pair, blind to oriented data the intermediate word-trace may carry.
...
\begin{remark}[Where the contribution lies]\label{rem:terminal}
The conclusion of Theorem~\ref{thm:terminal} is, in isolation, a
comparatively direct application of Lemma~\ref{lem:levelsets}: any
functional factoring through the reduced norm is constant on conjugation
orbits, and non-central conjugation classes in $\HH(\QQ)$ are classified by
$(\Trd,\Nrd)$. The substantive contribution is not the classification
itself but its \emph{location} in the algorithmic pipeline
tensor~$\to$~contraction~$\to$~word-trace~$\to$~$\Nrd$: the object and
the trace can carry oriented non-commutative data, yet the terminal
$\Nrd$ readout is exactly the step at which that data is projected to the
commutative central pair. Identifying this abelianization wall as the
precise failure point of the construction required building and analysing
the full chain; it was not manifest from the abstract quaternionic
formalism alone.
\end{remark}
...
\subsection{Illustrative connection to barrier literature}

Theorem~\ref{thm:terminal} illustrates, in miniature and with complete rigor,
a collapse mechanism we compare---only by structural analogy, without
claiming a formal reduction---to the algebrization barrier of Aaronson and
Wigderson~\cite{AW}. We do not establish any new theorem about
algebrization, nor show that our readout falls under the hypotheses of
that framework. The construction was
...
commutative extension---reminiscent, at the level of metaphor, of the
low-degree commutative summaries against which the algebrization program
is designed to guard~\cite{AW}, and, via Razborov--Rudich~\cite{RR}, of the
regime in which a smoothly varying separating functional would constitute a
natural property. The failure mode is not that the object is too simple; it is that the
\emph{measurement} is abelian.

\newpage

%=======================================================================
\section{Exact Verification on Size-Matched Instances}\label{sec:experiments}
%=======================================================================

\subsection{Methodology}

The experiments below \emph{corroborate} the structural impossibility
predictions of Sections~\ref{sec:collapse} and~\ref{sec:terminal}; they are
not a heuristic search for separation. The Quaternionic Invariant Laboratory
(QIL, Section~\ref{sec:software})
...
%=======================================================================
\section{Implications for Non-Commutative Obstruction Search}
\label{sec:discussion}
%=======================================================================

\subsection{Scope and readership}\label{sec:scope}

For the algebraic complexity reader, the take-away is a \emph{delimitation
theorem}, not a hardness result. We rule out a concrete class of local,
low-degree, quaternionic invariants---those obtained from bipartite
order-$2$ Dieudonn\'e data or from order-$3$ tensors read out through
$g\circ\Nrd$ after linear contraction and word-traces---as candidates for
$\QQ$-valued separation of exactly labelled $3$-SAT instances. The
$3$-CNF testbed supplies exact labels and finite verification; the
mathematical content is the two collapse mechanisms and the necessary
conditions they impose on successors. QIL (Section~\ref{sec:software})
certifies every rational constant; it does not alter the logical status of
the theorems.

The extrinsic collapse of Theorem~\ref{thm:terminal} is offered only as a
structural analogy with the algebrization barrier~\cite{AW}, without
claiming a formal reduction. Constraints for GCT-style obstruction
searches~\cite{GCT1,GCT2}---avoid two-colourable bilinear formats and
terminally abelianized readouts---follow from our proofs; they do not, by
themselves, advance the GCT program toward $\mathrm{P}$ versus $\mathrm{NP}$.

\subsection{Two poles of collapse}\label{sec:poles}
...
\subsection{Mathematical requirements for escape}\label{sec:escape}
...
$\HH(\QQ)//\HH(\QQ)^{\times}\cong\QQ^2$. This is orthogonal to, but
instructive when read alongside, the non-commutative rank and rational
identity literature~\cite{GGOV,IvanyosQiaoSubrahmanyam}: those frameworks
seek $\QQ$-certificates from non-commutative data by different means, whereas
our negative result isolates a specific Dieudonn\'e- and semi-invariant-style
pipeline~\cite{DerksenMakam} whose terminal reduced-norm readout is
intrinsically too coarse. Two structural directions are
...
%=======================================================================
\section{The Quaternionic Invariant Laboratory (QIL)}\label{sec:software}
%=======================================================================

QIL is the open-source exact-arithmetic environment in which every
quaternionic computation, invariant evaluation, and reproducible check in
this paper was carried out. Its role is \emph{verification}: the theorems are
proved analytically, and QIL certifies that each cited rational constant,
table entry, and pinned observation matches the formal definitions. The
complete workspace---Rust library, experiment drivers, and
documentation---is publicly available at
\href{\QILRepositoryURL}{\nolinkurl{\QILRepositoryURL}}.

\subsection{Architecture and correspondence to the paper}

QIL is a Rust library implementing, exactly, the mathematical objects of
Sections~\ref{sec:incidence}--\ref{sec:terminal}. Each module mirrors a paper
concept: the Hamiltonian division ring $\HH(\QQ)$; the incidence matrix and
order-$3$ hypergraph tensor; Dieudonn\'e and word-trace invariants; and the
\texttt{collapse} module encoding both impossibility mechanisms so successor
candidates can be tested against them before publication.
...
arbitrary-precision integers and rationals. Second, randomness is a
deterministic \texttt{SplitMix64} stream with fixed seeds. Third,
satisfiability labels come from exhaustive exact search over all $2^n$
assignments. Section~\ref{sec:experiments} tables are emitted verbatim by
\[
  \texttt{cargo run -{}-example reproduce\_table\_1},\qquad
  \texttt{cargo run -{}-example reproduce\_table\_2},
\]
with figures and pinned constants re-verified by
\texttt{reproduce\_figures} and \texttt{exact\_validation}.

\subsection{Extensibility}

QIL remains the maintained platform for testing successor invariants against
both collapse barriers---two-colourable bilinear formats and terminally
abelianized readouts---before they enter the analytical pipeline of future
work.
...
We have proved exact boundary theorems for a natural family of local
quaternionic invariants. Bipartite order-$2$ encodings suffer intrinsic gauge
collapse (Theorems~\ref{thm:collapse}, \ref{thm:intrinsic}); the minimal
arity lift to order-$3$ tensors removes that symmetry
(Proposition~\ref{prop:nogauge}) yet terminally abelianized scalar readouts
collapse again (Theorem~\ref{thm:terminal}, Remark~\ref{rem:terminal}).
Together these yield a taxonomy---failure in the object versus failure in
the measurement---and necessary conditions for any successor
(Section~\ref{sec:escape}). Exact data in Section~\ref{sec:experiments}
corroborates the predictions; QIL certifies every constant.

This is a delimitation result for algebraic complexity and obstruction
search, not a step toward resolving $\mathrm{P}$ versus $\mathrm{NP}$. The
sharply posed continuation is whether a readout that avoids both collapses
---non-abelian boundary evaluation or non-local contraction sensitive to
cross-clause sign interference---can produce an explicit obstruction for
$\Tphi$ or a related non-commutative carrier.
